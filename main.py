...
def create_an_student(req: Student):
    db_student = db.query(models.Student).filter(
        models.Student.student_F_Name == Student.student_F_Name).first()
...
def update_an_student(student_id: int, student: Student):
    student_to_update = db.query(models.Student).filter(
        models.Student.id == student_id).first()
...
def delete_student(student_id: int):
    student_to_delete = db.query(models.Student).filter(
        models.Student.id == student_id).first()
